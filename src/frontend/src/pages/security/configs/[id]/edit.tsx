...
const EditSecurityConfig: React.FC = () => {
  ...
  useEffect(() => {
    const fetchConfig = async () => {
      ...
      try {
        const config = await getSecurityConfig(id as string);
        setFormData(config);
      } catch (err) {
  ...
  const handleZoneInputChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
    ...
    setFormData(prev => ({
      ...prev,
      networkZones: [
        ...prev.networkZones.map(zone =>
          zone.name === name ? { ...zone, [name]: value } : zone
        ),
        {
          name: value,
          subnet: '',
  ...
  const handleAddZone = () => {
    if (!formData.networkZones) return;
    setFormData({
      ...formData,
      networkZones: [
        ...formData.networkZones,
        {
          name: '',
          subnet: '',
          securityLevel: 'high'
            ...
            <div className="grid grid-cols-1 gap-6 sm:grid-cols-3 mb-6">
              <div>
                ...
                <input
                  ...
                  id="zoneName"
                  name="name"
                  value={formData.networkZones?.find(z => z.name)?.name}
              ...
              <div>
                ...
                <input
                  ...
                  id="zoneSubnet"
                  name="subnet"
                  value={formData.networkZones?.find(z => z.name)?.subnet}
                  onChange={handleZoneInputChange}
                  className="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm"
                />
              </div>
              ...
              <div>
                ...
                <select
                  id="securityLevel"
                  name="securityLevel"
                  value={formData.networkZones?.find(z => z.name)?.securityLevel}
                  onChange={handleZoneInputChange}
                  className="mt-1 block w-full pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm rounded-md"
                >
